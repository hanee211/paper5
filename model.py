...
class Model():
	def __init__(self, params, training=True):
		...
		self.C = [tf.Variable(tf.ones([3])) for _ in range(total_size)]
		...
		for _ in range(num_layers):
			_cell = tf.contrib.rnn.GRUCell(num_units=rnn_size, input_size=rnn_size)
		...
		decoder_outputs, decoder_states = tf.contrib.legacy_seq2seq.rnn_decoder(self.decoder_input, self.decorder_initial_state if not training else self.encoder_final_states, decoder_cell, loop_function=loop if not training else None, scope=None)
